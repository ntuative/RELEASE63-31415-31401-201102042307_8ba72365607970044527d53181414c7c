package com.sulake.habbo.window
{
   ...
   public class HabboWindowManagerComponent extends Component implements IHabboWindowManager, ICoreWindowManager, IWindowFactory, IUpdateReceiver, IWindowContextStateListener
   {
      ...
      private static const const_409:uint = 3;
      
      private static const const_1249:uint = 1;
      ...
      private var var_851:IWindowRenderer;
      
      private var var_850:ISkinContainer;
      
      private var var_333:Array;
      
      private var var_1326:IEventDispatcher;
      
      private var var_1072:Boolean = false;
      
      private var var_1627:ProfilerOutput;
      
      private var var_1328:ProfilerAgentTask;
      ...
      private function initialize(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_850 = new SkinContainer(_loc4_.content as XML,assets);
         this.var_851 = new WindowRenderer(this.var_850);
         this.var_191 = new Array(const_409);
         ...
         var _loc6_:int = 0;
         while(_loc6_ < const_409)
         {
            this.var_191[_loc6_] = new WindowContext("layer_" + _loc6_,this.var_851,this,this._localization,context.displayObjectContainer,_loc5_,this);
            _loc6_++;
         }
         this._windowContext = this.var_191[const_1249];
         _loc7_ = new Bitmap(assets.getAssetByName("habbo_cursor_link_png").content as BitmapData);
         this._windowContext.getMouseCursor().defineCustomCursorType(MouseCursorType.const_370,_loc7_);
         _loc7_ = new Bitmap(assets.getAssetByName("habbo_cursor_drag_png").content as BitmapData);
         this._windowContext.getMouseCursor().defineCustomCursorType(MouseCursorType.const_1394,_loc7_);
         unlock();
      ...
      private function receiveProfilerInterface(param1:IID, param2:IUnknown) : void
      {
         ...
         if(_loc3_ != null)
         {
            if(!this.var_1627)
            {
               this.var_1627 = new ProfilerOutput(context,this,this.var_851);
            }
            this.var_1627.profiler = _loc3_;
            this.var_1328 = new ProfilerAgentTask("Update","Event processing");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1328);
            this.var_1327 = new ProfilerAgentTask("Redraw","Window rasterizer");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1327);
            this.var_1072 = true;
         }
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            ...
            if(this.var_191)
            {
               while(this.var_191.length > 0)
               {
                  IDisposable(this.var_191.pop()).dispose();
               }
            }
            this.var_191 = null;
            if(this.var_333)
            {
               while(this.var_333.length > 0)
               {
                  this.var_333.pop();
               }
            }
      ...
      {
         var _loc3_:Dictionary = this.var_333[param2] as Dictionary;
         if(_loc3_ == null)
         {
            return null;
         }
         var _loc4_:XML = _loc3_[param1] as XML;
         return _loc4_ == null ? this.var_333[0][param1] as XML : _loc4_;
      }
      
      public function getDefaultsByTypeAndStyle(param1:uint, param2:uint) : DefaultAttStruct
      {
         return this.var_850.getDefaultAttributesByTypeAndStyle(param1,param2);
      }
      ...
      {
         return this.var_191[param9].create(param1,param2,param3,param4,param5,param6,param7,null,param8,null,null);
      }
      
      public function removeWindow(param1:String, param2:uint = 1) : void
      {
         var _loc3_:IDesktopWindow = this.var_191[param2].getDesktopWindow();
      ...
      {
         return this.var_191[param2].getDesktopWindow().getChildByName(param1);
      }
      
      public function getActiveWindow(param1:uint = 1) : IWindow
      {
         return this.var_191[param1].getDesktopWindow().getChildAt(this._windowContext.getDesktopWindow().numChildren - 1);
      }
      ...
      {
         var _loc2_:IWindowContext = this.var_191[param1];
      ...
      public function update(param1:uint) : void
      {
         ...
         if(WindowContext.var_370.length > 0)
         {
            this.var_1326.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_INPUT));
            _loc2_ = const_409 - 1;
            while(_loc2_ >= 0)
            {
               this.var_191[_loc2_].update(param1);
               _loc2_--;
            }
         }
         if(this.var_1072)
         ...
         this.var_1326.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_RENDER));
         _loc2_ = 0;
         while(_loc2_ < const_409)
         {
            this.var_191[_loc2_].render(param1);
            _loc2_++;
         }
         if(this.var_1072)
         {
         ...
         if(WindowContext.var_370.length > 0)
         {
            WindowContext.var_370.flush();
         }
         this.var_1326.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_SLEEP));
      }
      ...
      public function mouseEventReceived(param1:String, param2:IWindow) : void
      {
         if(param2 != null)
         {
            ...
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1627,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1407,param2.name + ": " + param2.toString());
            }
            else if(param1 == MouseEvent.MOUSE_UP)
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1614,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1455,param2.name + ": " + param2.toString());
            }
         }
      }
      ...
      private function performTestCases() : void
      {
         ...
         _loc1_.begin(this);
         _loc1_.test({
            "type":WindowType.const_517,
            "style":WindowStyle.const_192,
            "caption":"Testing",
            "count":800,
            "properties":[_loc2_]
         });
