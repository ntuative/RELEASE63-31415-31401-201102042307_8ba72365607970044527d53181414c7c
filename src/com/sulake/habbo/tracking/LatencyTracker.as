package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      public function init() : void
      {
         ...
         this.var_1882 = int(this.var_180.getKey("latencytest.interval"));
         this.var_1884 = int(this.var_180.getKey("latencytest.report.index"));
         this.var_2654 = int(this.var_180.getKey("latencytest.report.delta"));
         this._communication.addHabboConnectionMessageEvent(new LatencyPingResponseMessageEvent(this.onPingResponse));
         if(this.var_1882 < 1)
         {
            return;
         }
         this.var_633 = new Map();
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1883 > this.var_1882)
         {
            this.testLatency();
         }
      }
      ...
      {
         this.var_1883 = getTimer();
         this.var_633.add(this.var_1448,this.var_1883);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1448));
         ++this.var_1448;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_633 == null || this.var_224 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_633.getValue(_loc2_.requestId);
         this.var_633.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_224.push(_loc4_);
         if(this.var_224.length == this.var_1884 && this.var_1884 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1885) > this.var_2654 || this.var_1885 == 0)
            {
               this.var_1885 = _loc9_;
