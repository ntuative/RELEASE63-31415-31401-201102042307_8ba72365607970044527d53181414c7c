package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1198:Dictionary;
      ...
      private var var_264:IRoomObjectSprite;
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1198)
         {
            _loc1_.dispose();
         }
         this.var_1198 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_93 = this.var_1198[param1];
         this.var_964 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1894 >= 0)
         {
            this.var_264 = this._visualization.getSprite(this.var_1894);
            if(this.var_264 && this.var_264.asset)
            {
               if(this.var_264.width <= 1 || this.var_264.height <= 1)
               {
                  return;
               }
               if(this.var_84 == null)
               {
                  this.var_84 = this.var_264.asset.clone();
                  if(this.var_773.alphaMultiplier != 1)
                  {
                     this.var_636 = new BitmapData(this.var_84.width,this.var_84.height,true,4278190080);
                  }
               }
               this.var_2345 = -this.var_264.offsetX;
               this.var_2346 = -this.var_264.offsetY;
               this.var_264.asset = this.var_84;
            }
            if(this.var_84)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_93.y * this.var_1199;
         }
         return 0;
      }
      ...
      {
         if(!this.var_93 || !this.var_264)
         {
            return;
         }
         if(this.var_84 && this.var_264.asset != this.var_84)
         {
            this.var_264.asset = this.var_84;
         }
         if(this.var_964)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_964 && this.var_93.hasIgnited)
         {
            this.var_964 = true;
         }
         _loc8_ = this.var_1023 * this.var_1199;
         this.var_93.update();
         if(this.var_964)
         {
            if(this.var_93.roomObjectSpriteId >= 0)
            ...
            {
               this.var_84.draw(this.var_636,this.var_1895,this.var_773,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_93.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2345 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1199;
               _loc4_ = this.var_2346 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1199;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1455.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_84.draw(_loc7_,this._translationMatrix,this.var_1455,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2674 = parseInt(param1.@size);
         this.var_1894 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1023 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1199 = this.var_2674 / 64;
         this.var_656 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_656 = Math.min(this.var_656,1);
         this.var_773.alphaMultiplier = this.var_656;
