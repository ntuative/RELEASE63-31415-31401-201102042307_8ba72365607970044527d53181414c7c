package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_996:BitmapData = null;
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2039 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_2040 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2038 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1505 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_202 != null && this.var_202.running)
         {
            this.var_202.reset();
            return false;
         }
         if(this.var_997 == 0)
         {
            this.var_997 = getTimer();
            return false;
         }
         if(this.var_1505 > 0 && this.var_1504 >= this.var_1505)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_997;
         if(_loc1_ > this.var_2040)
         {
            ++this.var_1504;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1247 = _loc2_.imageUrl;
         this.var_2037 = _loc2_.clickUrl;
         this.var_997 = getTimer();
         if(this.var_1247 != "")
         {
            _loc3_ = this.var_1247;
            ...
            {
               this.var_202 = new Timer(this.var_2038,1);
               this.var_202.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_202.running)
            {
               this.var_202.reset();
            }
            this.var_202.start();
            _loc4_ = new URLRequest(this.var_1247);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_420,0,0,param1.bitmapData,this.var_2037));
            this.var_997 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_576));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1248 = _loc2_.imageUrl;
         this.var_1506 = _loc2_.clickUrl;
         if(this.var_1248 != "" && this.var_1506 != "")
         {
            _loc3_ = this.var_1248;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1248);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_534,0,0,_loc4_,this.var_1506,this.var_996,this.var_998));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_996 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_998 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
