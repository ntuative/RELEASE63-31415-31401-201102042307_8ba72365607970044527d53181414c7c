package com.sulake.habbo.help.hotelmerge
{
   ...
   public class HotelMergeUI implements INameChangeUI
   {
      ...
      {
         this.var_179.sendMessage(new ChangeUserNameMessageComposer(param1));
      }
      
      public function checkName(param1:String) : void
      {
         this.var_179.sendMessage(new CheckUserNameMessageComposer(param1));
      }
      ...
      public function onUserNameChanged(param1:String) : void
      {
         ...
         this.var_179.localization.registerParameter("help.tutorial.name.changed","name",name);
         this.var_179.windowManager.alert("${generic.notice}","${help.tutorial.name.changed}",0,function(param1:IAlertDialog, param2:Event):void
         {
            param1.dispose();
         });
      }
      ...
      {
         if(this.var_179 == null || this.var_179.assets == null)
         {
            return null;
         }
         var _loc3_:XmlAsset = XmlAsset(this.var_179.assets.getAssetByName(param1 + "_xml"));
         if(_loc3_ == null || this.var_179.windowManager == null)
         {
            return null;
         }
         var _loc4_:ICoreWindowManager = ICoreWindowManager(this.var_179.windowManager);
