package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_558:String = "InitConnection";
       
      
      private var var_140:HabboCommunicationDemo;
      ...
      {
         this.var_140.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_140 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_140.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_132,HabboWindowStyle.const_39,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_140.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1653,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_140.habboConfiguration)
            {
               if(this.var_140.habboConfiguration.getKey("use.sso","false") == "true")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_140.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_140.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1653,"/");
         ...
         {
            dispatchEvent(new Event(const_558));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_140.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_120.removeEventListener(SSOTicket.const_397,this.onParsedTicketSuccess);
            this.var_120.removeEventListener(SSOTicket.const_313,this.onParsedTicketFailure);
            this.var_120 = null;
         }
         var _loc2_:String = "";
         if(this.var_140.habboConfiguration != null)
         {
            _loc2_ = this.var_140.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_140.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_120 = new SSOTicket(this.var_140.assets,_loc3_,_loc2_,param1);
         this.var_120.addEventListener(SSOTicket.const_397,this.onParsedTicketSuccess);
         this.var_120.addEventListener(SSOTicket.const_313,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_120.ticket);
         Logger.log("Has Facebook: " + this.var_120.isFacebookUser);
         if(this.var_120.isFacebookUser)
         {
            this.var_140.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_140.shockwaveClientUrl = this.var_120.shockwaveClientUrl;
         this.var_140.ssoTicket = this.var_120.ticket;
         this.var_120.removeEventListener(SSOTicket.const_397,this.onParsedTicketSuccess);
         this.var_120.removeEventListener(SSOTicket.const_313,this.onParsedTicketFailure);
         this.var_120 = null;
         dispatchEvent(new Event(const_558));
