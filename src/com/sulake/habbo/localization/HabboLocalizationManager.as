package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private var var_1744:Dictionary;
      ...
      {
         this.var_1744 = new Dictionary();
         this.var_2471 = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV","XV","XVI","XVII","XVIII","XIX","XX"];
         super(param1,param2,param3);
         this.var_1743 = (param2 & 0) > 0;
         if(!this.var_1743)
         {
            lock();
         }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            _loc3_ = "";
            _loc3_ = this.var_180.getKey("url.prefix",_loc3_);
            _loc3_ = _loc3_.replace("http://","");
            _loc3_ = _loc3_.replace("https://","");
            super.setPredefinedUrl(_loc3_);
            if(!this.var_1743)
            {
               if("false" == this.var_180.getKey("use.default.localizations","false"))
               {
                  _loc5_ = this.var_180.getKey("external.texts.txt","external_texts.txt");
                  events.addEventListener(LocalizationEvent.const_159,this.onLocalizationLoaded);
                  super.loadLocalizationFromURL(_loc5_);
               }
            }
            _loc4_ = 1;
            while(this.var_180.keyExists("localization." + _loc4_))
            {
               _loc6_ = this.var_180.getKey("localization." + _loc4_,"");
               _loc7_ = this.var_180.getKey("localization." + _loc4_ + ".name","");
               _loc8_ = this.var_180.getKey("localization." + _loc4_ + ".url","");
      ...
      public function onLocalizationLoaded(param1:Event) : void
      {
         ...
         if(this.var_180.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_180.getKey("external.override.texts.txt");
      ...
      {
         this.var_1744[param1] = param2;
      }
      
      private function getBadgePointLimit(param1:String) : int
      {
         return this.var_1744[param1];
      }
      ...
      {
         return this.var_2471[Math.max(0,param1 - 1)];
      }
   }
}
