package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_397:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_351);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_397.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_397.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_397.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_397.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_397.view.setNewMessageArrived();
                  this.var_397.notifications.method_9(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_351).include = true;
         this.findCategory(FriendCategory.const_1184).include = true;
         ...
         for each(_loc3_ in this.getCategoriesNotReceived())
         {
            ...
            {
               Util.remove(this.var_94,_loc3_);
               _loc3_.dispose();
            }
         }
      }
