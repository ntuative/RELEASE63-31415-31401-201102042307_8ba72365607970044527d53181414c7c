package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1751:int = 1;
      
      public const const_1783:int = 2;
      
      public const const_1989:int = 3;
      ...
      private var var_2058:MarketplaceItemStats;
      ...
      public function dispose() : void
      {
         ...
         if(this.var_166)
         {
            for each(_loc1_ in this.var_166)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
         ...
         if(this.var_203)
         {
            for each(_loc1_ in this.var_203)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
      ...
      {
         this.requestOffers(this.var_2059,this.var_2056,this.var_2055,this.var_2057);
      }
      ...
      {
         if(!this.var_166 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_166.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_1751,_loc2_);
      }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = this.var_203.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = this.var_203.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_435)
            {
               this.var_203.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_166)
         {
            for each(_loc5_ in this.var_166)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            this.var_166.add(_loc4_.offerId,_loc6_);
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_203)
         {
            for each(_loc5_ in this.var_203)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         this.var_2063 = _loc3_.creditsWaiting;
         this.var_203 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            this.var_203.add(_loc4_.offerId,_loc6_);
         }
         if(this._visualization == null)
         {
            return;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_166.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_166.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_166.add(parser.offerId,updateItem);
            }
            this.var_166.remove(parser.requestedOfferId);
            this.showConfirmation(this.const_1783,updateItem);
            this._visualization.listUpdatedNotify();
         }
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = this.var_203.remove(parser.offerId);
            if(item != null)
            {
               item.dispose();
            }
      ...
      {
         if(param1.furniCategoryId != this.var_2062 || param1.furniTypeId != this.var_2061)
         {
            return;
         }
      ...
      private function isPosterItem(param1:IMarketPlaceOfferData) : Boolean
      {
         ...
         {
            _loc3_ = this._catalog.getFurnitureData(param1.furniId,ProductTypeEnum.const_82);
            if(_loc3_)
            {
               _loc4_ = _loc3_.name;
               if(_loc4_ != null && _loc4_ == const_1214)
               {
                  _loc2_ = true;
               }
            }
