package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_945 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_366 = [];
         this.var_637 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_945,this);
               if(_loc9_)
               {
                  this.var_637 = _loc9_.sellableBreeds;
                  this.var_366 = _loc9_.colors;
         ...
         {
            _loc10_ = this.var_637[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_945,_loc10_.id),this.getRaceLocalizationKey(this.var_945,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_945,_loc5_);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_366)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_517 = param1.index;
         if(this.var_517 < 0 || this.var_517 > this.var_366.length)
         {
            this.var_517 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_517 >= 0 && this.var_517 < this.var_366.length)
         {
            _loc7_ = this.var_366[this.var_517] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_637[this.var_506] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_945,_loc3_.id,_loc2_,AvatarScaleType.const_54,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_302,AvatarAction.const_689);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_517 >= this.var_366.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_366[this.var_517] as IPetColor;
