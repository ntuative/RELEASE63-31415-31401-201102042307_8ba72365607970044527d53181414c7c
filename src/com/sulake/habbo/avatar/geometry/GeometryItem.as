package com.sulake.habbo.avatar.geometry
{
   public class GeometryItem extends Node3D
   {
      ...
      public function GeometryItem(param1:XML, param2:Boolean = false)
      {
         ...
         this.var_1401 = parseFloat(param1.@radius);
         this.var_540 = new Vector3D(parseFloat(param1.@nx),parseFloat(param1.@ny),parseFloat(param1.@nz));
         this.var_2680 = parseInt(param1.@double) as Boolean;
         this.var_2679 = param2;
      }
      
      public function getDistance(param1:Vector3D) : Number
      {
         var _loc2_:Number = Math.abs(param1.z - this.transformedLocation.z - this.var_1401);
         var _loc3_:Number = Math.abs(param1.z - this.transformedLocation.z + this.var_1401);
