package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_970:int = 3;
      
      private var var_429:AssetLoaderStruct;
      
      private var var_972:Timer;
      ...
      private const const_1983:int = 100;
      
      private const const_1352:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_971 = new Dictionary();
         this.var_2712 = param3;
         ...
         if(_loc6_ == null)
         {
            this.var_429 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_972 = new Timer(this.const_1983,1);
         this.var_972.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_429)
         {
            this.var_429.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_429.addEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_429)
         {
            this.var_429.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_429.removeEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_970);
            if(this.var_1470.indexOf("?") > 0)
            {
               _loc2_ = this.var_1470 + "&retry=" + this.var_970;
            }
            else
            {
               _loc2_ = this.var_1470 + "?retry=" + this.var_970;
            ...
            this.var_429 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1919 = true;
         for each(_loc2_ in this.var_1208)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2712,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1209[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1919)
         {
            this.var_1208.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_458[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_458[_loc3_] = _loc6_;
            }
            this.var_971[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_971)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_458[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_971[_loc3_];
         }
         while(_loc6_ < this.var_781.length)
         {
            _loc5_ = this.var_781[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_781.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_972.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_780.length > 0 && this.var_781.length < this.const_1352)
         {
            _loc1_ = this.var_780[0];
            _loc1_.startDownloading();
            this.var_781.push(this.var_780.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_780.indexOf(param1) == -1 && this.var_781.indexOf(param1) == -1)
         {
