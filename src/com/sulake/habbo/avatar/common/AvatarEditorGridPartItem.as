package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private static var var_56:Array = [];
      ...
      private var var_421:IAvatarRenderManager;
      
      private var var_724:Boolean;
      
      private var var_1847:BitmapData;
      
      private const const_1974:int = 13421772;
      
      private const const_1973:int = 16777215;
      
      private const const_327:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_647 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1848 = Math.max(this.var_1848,_loc6_.colorLayerIndex);
            }
         }
         this.var_421 = this.var_65.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_647 != null && !this.var_1849)
            {
               _loc3_ = this.var_647;
            }
            else
            ...
            if(this.var_753)
            {
               switch(this.var_753.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_62:
                     _loc2_.style = HabboIconType.const_740;
                     break;
                  case HabboClubLevelEnum.const_55:
                     _loc2_.style = HabboIconType.const_867;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1974;
         }
         else
         {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_421.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_421.isFigureReady(_loc1_))
         {
            this.var_421.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_327[_loc6_] + "_" + FigureData.const_132;
               _loc4_ = this.var_421.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_327.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_327[_loc6_] + "_" + FigureData.const_132;
                  _loc4_ = this.var_421.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_622 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1847)
               {
                  ...
                  this.var_1847 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1849 && _loc2_.colorLayerIndex > 0)
               {
      ...
      {
         var _loc3_:Number = var_56.indexOf(param1.type);
         var _loc4_:Number = var_56.indexOf(param2.type);
         if(_loc3_ < _loc4_)
         {
            return -1;
         }
