package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1679:int = 60000;
       
      
      private var _structure:AvatarStructure;
      
      private var var_1370:IAvatarImage;
      ...
      private var var_336:IActiveActionData;
      
      private var var_409:AvatarImageBodyPartContainer;
      
      private var var_1337:String;
      
      private var var_106:AvatarImagePartContainer;
      
      private var var_2317:Point;
      
      private var var_704:String;
      
      private var var_175:BitmapData;
      ...
      private var var_399:BitmapDataAsset;
      
      private var var_866:BitmapData;
      
      private var var_2316:ByteArray;
      
      private var var_2318:ImageData;
      
      private var var_888:PaletteMap;
      
      private var var_867:Array;
      
      private var var_868:Point;
      ...
      {
         if(this.var_704 == param1)
         {
            return;
         }
         if(this.var_704 == GeometryType.const_1146 && param1 == GeometryType.const_1048 || this.var_704 == GeometryType.const_1048 && param1 == GeometryType.const_1146)
         {
            this.var_704 = param1;
            this.var_84 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         this.var_336 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_336;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_336 || !this.var_336.definition))
         {
            if(this.var_336.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_336.definition.state + "." + this.var_336.actionParameter);
               _loc13_ = param2 - this.var_336.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_336.overridingAction);
                  if(_loc14_ != null)
                  {
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1370.getFigure(),_loc5_,this.var_704,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_409 = _loc10_.getImageContainer(_loc4_);
         if(this.var_409 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_409 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_336);
            if(this.var_409 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_409,_loc4_);
         }
         this.var_409.offset = _loc8_;
         return this.var_409;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_84)
         {
            this.var_84 = this._structure.getCanvas(this._scale,this.var_704);
            if(!this.var_84)
            {
               return null;
            }
            this.var_2317 = new Point(0,this.var_84.height - int(this.var_84.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1636[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_820 || _loc10_ == AvatarAction.const_348;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1337 = this.var_106.bodyPartId;
               this.var_174 = this.var_106.partType;
               this.var_1656 = this.var_106.partId;
               this._frame = this.var_106.getFrameIndex(param3);
               _loc5_ = param1;
               _loc7_ = false;
               if(_loc6_)
               {
                  if(_loc9_ == "wav" && (this.var_174 == "lh" || this.var_174 == "ls" || this.var_174 == "lc" || this.var_174 == "lhs"))
                  {
                     _loc7_ = true;
                  }
                  else if(_loc9_ == "drk" && (this.var_174 == "rh" || this.var_174 == "rs" || this.var_174 == "rc" || this.var_174 == "rhs"))
                  {
                     _loc7_ = true;
                  }
                  else if(_loc9_ == "crr" && this.var_174 == "rhs")
                  {
                     _loc7_ = true;
                  }
                  else if(this.var_174 == "ri")
                  {
                     _loc7_ = true;
                  }
                  else if(this.var_174 == "cp")
                  {
                     _loc7_ = true;
                  }
                  else
                  {
                     ...
                     if(this.var_106.flippedPartType != this.var_174)
                     {
                        this.var_174 = this.var_106.flippedPartType;
                     }
                  }
               }
               this.var_1338 = this._scale + "_" + this.var_106.action.assetPartDefinition + "_" + this.var_174 + "_" + this.var_1656 + "_" + _loc5_ + "_" + this._frame;
               this.var_399 = this._assets.getAssetByName(this.var_1338) as BitmapDataAsset;
               if(this.var_399 == null)
               {
                  this.var_1338 = this._scale + "_std_" + this.var_174 + "_" + this.var_1656 + "_" + _loc5_ + "_0";
                  this.var_399 = this._assets.getAssetByName(this.var_1338) as BitmapDataAsset;
               }
               if(!(!this.var_399 || !this.var_399.content))
               {
                  if(this.var_106.paletteMapId > -1)
                  {
                     this.var_888 = this._structure.method_7.paletteManager.getPalette(this.var_106.paletteMapId);
                     if(this.var_888 != null)
                     {
                        this.var_866 = this.var_399.content as BitmapData;
                        this.var_175 = new BitmapData(this.var_866.width,this.var_866.height);
                        this.var_2316 = this.var_888.colorize(this.var_866.getPixels(this.var_866.rect));
                        this.var_175.setPixels(this.var_175.rect,this.var_2316);
                     }
                     else
                     {
                        this.var_175 = (this.var_399.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1337 == AvatarBodyPartType.const_247 || this.var_1337 == AvatarBodyPartType.const_1520 || this.var_1337 == AvatarBodyPartType.const_1389) && this.var_106.color != null)
                  {
                     _loc16_ = 0.7;
                     ...
                     this.var_175.colorTransform(this.var_175.rect,_loc18_);
                  }
                  else if(this.var_106.isColorable && this.var_106.color != null)
                  {
                     this.var_175.colorTransform(this.var_175.rect,this.var_106.color.colorTransform);
                  }
                  if(this.var_106.isBlendable)
                  {
                     this.var_175.colorTransform(this.var_175.rect,this.var_106.blendTransform);
                  }
                  this.var_868 = this.var_399.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_54)
                     {
                  ...
                  this.var_2318 = new ImageData(this.var_175,this.var_868,_loc7_);
                  this.var_867.push(this.var_2318);
               }
            }
            _loc8_--;
         }
         if(this.var_867.length == 0)
         {
            return null;
         }
         var _loc13_:ImageData = this.createUnionImage(this.var_867,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2317.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_54)
            {
         ...
         _loc8_ = this.var_867.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_867.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
