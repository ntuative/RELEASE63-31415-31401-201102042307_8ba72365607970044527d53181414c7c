package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_334,this.onQueueStatus);
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_509,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_334,this.onQueueStatus);
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_509,this.onQueueStatus);
      }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1484 = param1.type;
            this.var_1238 = false;
            this.var_985 = param1.position;
         }
         else
         {
            this.var_1238 = true;
         }
         this.var_2001 = param1.hasHabboClub;
         this.var_1485 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_985.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_985.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_985.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_985.toString());
         this.showInterface();
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(this.var_1484)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_334:
                  _loc1_.caption = !!this.var_1485 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1238;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_509:
                  _loc1_.caption = !!this.var_1485 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1238;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2001;
         }
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_784);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_622));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1484 == RoomWidgetRoomQueueUpdateEvent.const_334)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_853);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
