package com.sulake.room.renderer.cache
{
   ...
   public class BitmapDataCache
   {
      ...
      public function compress() : void
      {
         ...
         {
            this.var_398.sortOn("useCount",0 | 0);
            _loc2_ = this.var_398.length - 1;
            while(_loc2_ >= 0)
            {
               _loc1_ = this.var_398[_loc2_] as BitmapDataCacheItem;
            ...
            this.var_398.splice(_loc2_,this.var_398.length - _loc2_);
            if(this.memUsage > this.memLimit)
            {
               this.var_398.sortOn("memUsage",Array.NUMERIC);
               while(this.memUsage > this.memLimit)
               {
                  _loc1_ = this.var_398.pop() as BitmapDataCacheItem;
                  if(_loc1_ == null)
                  {
                     break;
                  }
      ...
      public function addBitmapData(param1:String, param2:ExtendedBitmapData) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            if(_loc4_ != null)
            {
               this.var_700 -= _loc4_.width * _loc4_.height * 4;
            }
         ...
         else
         {
            ...
            this.var_398.push(_loc3_);
         }
         this.var_700 += param2.width * param2.height * 4;
      }
   }
}
