package com.sulake.room.renderer.cache
{
   ...
   public class RoomObjectLocationCacheItem
   {
      ...
      public function getScreenLocation(param1:IRoomObject, param2:IRoomGeometry) : IVector3d
      {
         ...
         if(param2.updateId != this.var_1514 || param1.getUpdateID() != this.var_2292)
         {
            this.var_2292 = param1.getUpdateID();
            if(param2.updateId != this.var_1514 || _loc4_.x != this.var_1078.x || _loc4_.y != this.var_1078.y || _loc4_.z != this.var_1078.z)
            {
               this.var_1514 = param2.updateId;
               this.var_1078.assign(_loc4_);
               _loc3_ = true;
            }
         }
         this.var_2293 = _loc3_;
         if(_loc3_)
         {
            ...
            if(isNaN(_loc6_) || _loc6_ == 0)
            {
               ...
               if(_loc7_.x != _loc4_.x || _loc7_.y != _loc4_.y)
               {
                  ...
                  if(_loc8_ != null)
                  {
                     this.var_335 = new Vector3d(_loc5_.x,_loc5_.y,_loc8_.z);
                  }
                  else
                  {
                     this.var_335.assign(_loc5_);
                  }
               }
               else
               {
            ...
            this.var_335.x = Math.round(this.var_335.x);
            this.var_335.y = Math.round(this.var_335.y);
         }
         return this.var_335;
      }
   }
