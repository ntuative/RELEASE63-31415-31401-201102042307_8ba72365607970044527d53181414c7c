package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1646:uint = 1;
      
      protected static const const_1647:uint = 2;
      
      protected static const const_1645:uint = 4;
      
      protected static const const_1648:uint = 3;
      
      protected static const name_3:uint = 7;
      
      public static const const_1522:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1047:Boolean = false;
      
      public static const const_1352:int = 2;
      
      private static var var_901:Array = new Array();
      
      private static var var_730:Array = new Array();
      ...
      protected var var_374:int = 0;
      
      protected var var_228:URLRequest;
      
      protected var var_757:XML;
      
      protected var var_623:Class;
      
      protected var var_435:uint = 0;
      
      protected var _name:String;
      
      protected var var_977:Boolean = false;
      
      protected var _debug:Boolean = false;
      
      protected var var_978:Boolean = false;
      
      protected var var_2738:String = "";
      
      protected var var_2736:String = "";
      
      protected var var_1974:int;
      
      protected var var_1221:int;
      
      protected var var_1222:int;
      
      protected var var_2810:uint = 0;
      
      protected var var_2737:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_39.load(param1.var_228,param1.var_16);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1047)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_730[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_730.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_730.length < const_1352 && false)
            {
               _loc1_ = var_901.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_730.push(_loc1_);
                  _loc1_.var_39.load(_loc1_.var_228,_loc1_.var_16);
               }
            }
         }
      }
      ...
      {
         return !!this.var_228 ? this.var_228.url : null;
      }
      ...
      {
         return !!this.var_977 ? uint(this.var_1221 - this.var_1974) : uint(getTimer() - this.var_1974);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_374,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_39.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_228.url);
      ...
      {
         if(this.var_978 && !_disposed)
         {
            this.var_978 = false;
            if(!this.var_977 && this.var_228)
            {
               this.var_1221 = -1;
      ...
      {
         if(!this.var_977 && !_disposed && this.var_1222 > 0)
         {
            try
            {
               this.var_39.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_228,const_1522 - this.var_1222);
            if(this.var_2737)
            {
               this.var_228.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_228.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2737 = false;
            }
            --this.var_1222;
            this.var_435 = 0;
            this.var_39.load(this.var_228,this.var_16);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_374 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_374 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_374,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_374,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_374))
               {
                  this.var_1221 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_435 = LibraryLoader.name_3;
         ...
         if(this.var_435 == LibraryLoader.const_1648)
         {
         ...
         if(this.var_435 == LibraryLoader.name_3)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_374,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_623 = this.getDefinition(this._name) as Class;
         if(this.var_623 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_228.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_623.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
         ...
         try
         {
            this.var_757 = new XML(bytes.readUTFBytes(bytes.length));
         }
         catch(e:Error)
         {
            if(!retry())
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_374,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_374,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
