package com.sulake.core.runtime
{
   ...
   class Profiler extends Component implements IProfiler
   {
      ...
      private var var_307:Dictionary;
      ...
      {
         this.var_307 = new Dictionary(true);
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            for(_loc1_ in this.var_307)
            {
               IDisposable(this.var_307[_loc1_]).dispose();
               delete this.var_307[_loc1_];
            }
            super.dispose();
         }
      }
      ...
      {
         this.var_2778 = getTimer() - this.var_2352;
      ...
      public function getProfilerAgentsInArray() : Array
      {
         ...
         for(_loc3_ in this.var_307)
         {
            _loc2_ = _loc3_ as IUpdateReceiver;
            if(IDisposable(_loc3_).disposed)
            {
               IDisposable(this.var_307[_loc3_]).dispose();
               delete this.var_307[_loc3_];
            }
            else
            {
               _loc1_.push(this.var_307[_loc3_]);
            }
         }
         return _loc1_;
      }
      ...
      {
         var _loc2_:ProfilerAgent = this.var_307[param1];
